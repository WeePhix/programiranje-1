...
let dollar_to_euro d = match d with
| Dollar v -> Euro (v *. 0.95)

let euro_to_dollar e = match e with
| Euro v -> Dollar (v *. 1.06)
...
let to_pound curr = match curr with
| Yen v -> Pound (v *. 0.0051)
| Pound v -> curr
| Krona v -> Pound (v *. 0.072)
| Frank v -> Pound (v *. 0.89)
...
| Nil -> Nil
| Int (n, t) -> Int ((f_int n), (intbool_map f_int f_bool t))
| Bool (b, t) -> Bool ((f_bool b), (intbool_map f_int f_bool t))
...
let rec intbool_reverse ib_list = 
  ...
  | Nil -> acc
  | Int (n, t) -> aux t (Int (n, acc))
  | Bool (b, t) -> aux t (Bool (b, acc))
  in
  aux ib_list Nil
...
let rec intbool_separate ib_list =
  let rec aux l ints bools = match l with
  | Nil -> (ints, bools)
  | Int (n, t) -> aux t (ints @ [n]) bools
  | Bool (b, t) -> aux t ints (bools @ [b])
  in
  aux ib_list [] []
...
let professor = Employed (Fire, Teacher)

let jaina  = Student (Frost, 4)
