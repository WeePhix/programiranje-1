...
let read_instruction current_state =
  let ip = match current_state.ip with Address v -> v in
  if Array.length current_state.instructions <= ip then None else Some current_state.instructions.(ip)
...
let read_register current_state reg =
  match reg with
  | A -> current_state.a
  | B -> current_state.b
  | C -> current_state.c
...
let read_expression current_state inquired_expression =
  match inquired_expression with
  | Register reg -> read_register current_state reg
  | Const v -> v
...
let write_register current_state reg v = 
  match reg with
  | A -> {current_state with a = v}
  | B -> {current_state with b = v}
  | C -> {current_state with c = v}
  | D -> {current_state with d = v}
...
let perform_unop f current_state reg = write_register current_state reg (f (read_register current_state reg))
...
let perform_binop operation current_state reg expr = write_register current_state reg (operation (read_register current_state reg) (read_expression current_state expr))
...
let jump current_state addr = {current_state with ip = addr}
let proceed current_state = {current_state with ip = next current_state.ip}
...
let push_stack current_state v = {current_state with stack = (v :: current_state.stack)}
let pop_stack current_state =
  match current_state.stack with
  | [] -> failwith "Stack empty"
  | h::t -> h, {current_state with stack = t}
...
let compare state n m = {{state with zero = (n=m)} with carry = (n < m)}
...
let conditional_jump current_state addr condition =
  if condition then jump current_state addr
  else proceed current_state
...
let call current_state addr = 
  let ip = match current_state.ip with Address v -> v in
  push_stack (jump current_state addr) (ip + 1)
...
let return current_state =
  let addr, st = pop_stack current_state in
  {st with ip = Address addr}
...
let rec run_program st =
  match st.ip with
  | Address v when (Array.length st.instructions <= v) || (st.instructions.(v) = HLT) -> st
  | Address v -> run_program (run_instruction st st.instructions.(v))

let test = init (fibonacci 10)
let primer_izvajanje_16 =
  fibonacci 10
  |> init
  |> run_program
...
| reg when (reg = "A") || (reg = "B") || (reg = "C") || (reg = "D") -> Register (parse_register reg)
| const when Option.is_some (int_of_string_opt const) -> Const (int_of_string const)
...
let clean_line instr =
  let remove_comment str =
    match String.index_opt str ';' with
    | None -> str
    | Some n -> String.sub str 0 n
  in
  let rec remove_double_spaces str acc = 
    match str with
    | _ when Option.is_none (String.index_opt str ' ') -> acc ^ str
    | _ when String.get str 0 = ' ' -> remove_double_spaces (String.sub str 1 (String.length str -1)) acc
    | _ -> remove_double_spaces (String.sub str (String.index str ' ' + 1) (String.length str - String.index str ' ' - 1)) (acc ^ String.sub str 0 (String.index str ' ' + 1))
  in
  remove_double_spaces (String.trim (remove_comment instr)) ""
...
let clean_lines instr_list =
  let rec aux lines acc = 
    match lines with
    | [] -> acc
    | h::t -> aux t (acc @ [clean_line h])
  in
  List.filter (fun line -> line <> "") (aux instr_list [])
...
let parse_address labels label =
  let rec aux labels label = 
    match labels with
    | [] -> failwith "Tag not found"
    | (label_string, label_address)::t when label_string = label -> label_address
    | h::t -> aux t label
  in
  if Option.is_none (int_of_string_opt label) then aux labels label else Address (int_of_string label)
...
let parse_label label =
  match label with
  | _ when Option.is_none (String.index_opt label ':') -> None
  | _ when String.index label ':' = String.length label - 1 -> Some (String.sub label 0 (String.length label - 1))
  | _ -> None
...
let parse_labels lines =
  let rec aux lines index pairs insts =
    match lines with
    | [] -> pairs, insts
    | h::t when Option.is_none (parse_label h) -> aux t (index + 1) pairs (insts @ [h])
    | h::t -> aux t index ((Option.get (parse_label h), Address index) :: pairs) insts
  in
  aux lines 0 [] []
...
let run text =
  let lines = clean_lines (String.split_on_char '\n' text) in
  let (labels, instructions) = parse_labels lines in
  let instruction_list = List.map (parse_instruction labels) instructions in
  init instruction_list |> run_program
